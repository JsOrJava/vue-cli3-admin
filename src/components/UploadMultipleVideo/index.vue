...
<template>
  <div class="upload-file__wrapper clear">
    <template v-for="(ele, index) in faceUrlList">
      <div class="video-wh fl mr10">
        <video class="child" :src="ele.url" controls="controls">
          您的浏览器不支持 video 标签。
        </video>
        <i class="el-icon-circle-plus-outline expanded-icon u-btn__cursor--p" title="放大视频" @click="handlePictureCardPreview(ele)"></i>
        <i class="el-icon-delete delete-icon u-btn__cursor--p" title="删除视频" @click="handleRemove(ele)"></i>
      </div>
    </template>
    <el-upload
      class="fl mr10"
